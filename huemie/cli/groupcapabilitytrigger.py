"""Trigger capability on a group of devices"""
...
def argparse_expand(parser: argparse.ArgumentParser):
    """Sets up the command to trigger group capability"""
    parser.set_defaults(func=__devices_capability_trigger_main)
    parser.add_argument("--group", type=str, help="The group id", required=True)
    parser.add_argument(
...
def __devices_capability_trigger_main(args: argparse.Namespace):
    ...
    handle.post_json(
        path=f"device-store/v0/groups/{args.group}/capabilities/{args.capability}",
        data=arguments,
    )
